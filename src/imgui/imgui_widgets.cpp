...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool wrapped)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb, wrapped);
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, bool wrapped)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, wrapped);
}
